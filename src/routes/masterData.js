...
router.get("/", async (req, res) => {
  try {
    const [items, kiloan, perfumes, durations] = await Promise.all([
      ...
      pool.query(`SELECT code, name, time_label, multiplier FROM duration_multipliers ORDER BY multiplier`),
    ]);
    res.json({
      items: items.rows,
...
// PATCH /api/master-data/durations/:code — admin ubah multiplier durasi
router.patch("/durations/:code", authenticate, requireRole("admin", "owner"), async (req, res) => {
  const { multiplier } = req.body;
  try {
    const result = await pool.query(
      `UPDATE duration_multipliers SET multiplier = COALESCE($1, multiplier) WHERE code = $2 RETURNING *`,
      [multiplier, req.params.code]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: "Durasi tidak ditemukan." });
    res.json(result.rows[0]);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Gagal mengubah multiplier." });
  }
});
